...
public class JobContainer extends AbstractContainer {
    ...
    public void start() {
        ...
        try {
            ...
            if(isDryRun) {
                ...
                this.init();            //初始化Reader和Writer,并触发Reader和Writer方法里面的init方法
                LOG.info("jobContainer starts to do prepare ...");
                this.prepare();         //执行前，会触发Reader和Writer里面的prepare方法
                LOG.info("jobContainer starts to do split ...");
                this.totalStage = this.split();  //根据配置分割任务，计算需要分多少个任务和任务组
                LOG.info("jobContainer starts to do schedule ...");
                this.schedule(); // 开始调度，这里会进入TaskGroupContainer
                LOG.debug("jobContainer starts to do post ...");
                this.post();     //会触发Reader和Writer里面的post方法

                LOG.debug("jobContainer starts to do postHandle ...");
                this.postHandle();
                LOG.info("DataX jobId [{}] completed successfully.", this.jobId);

                this.invokeHooks();       //触发钩子，监控整个容器的运行情况
            }
        } catch (Throwable e) {
    ...
    private int split() {
        ...
        /**
         * 切分读插件，返回包含各个切分后的读插件配置列表，后续一个服务使用一个插件
         */
    ...
    /**
     * 计算needChannelNumber数量，这个变量为后面执行具体拆分task的依据，
     */
