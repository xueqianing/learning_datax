...
public class Engine {
    ...
    public void start(Configuration allConf) {
        ...
        /**
         * 判断任务的类型，从而创建TaskContainer或者JobContainer。
         */
        boolean isJob = !("taskGroup".equalsIgnoreCase(allConf
                .getString(CoreConstant.DATAX_CORE_CONTAINER_MODEL)));
        //JobContainer会在schedule后再行进行设置和调整值
        int channelNumber =0;
        AbstractContainer container;     //抽象容器，容器分两种：JobContainer 和 TaskGroupContainer
        long instanceId;
        int taskGroupId = -1;
        if (isJob) {
...
//            //设置datax的运行脚本信息
            args = new String[]{"-mode", "standalone", "-jobid", "-1", "-job", "H:\\learning_code_a\\DataX\\core\\src\\main\\bin\\demo.json"};
            /**
             * 程序入口
             */
            Engine.entry(args);
